...
object FunSets {
  ...
  def forall(s: Set, p: Int => Boolean): Boolean = {
    def iter(a: Int): Boolean = {
      if (a <= 1000 && contains(s, a)) p(a) && iter(a + 1)
  ...
  def exists(s: Set, p: Int => Boolean): Boolean = {
    def iter(a: Int): Boolean = {
      if (a <= bound && contains(s, a)) forall(singletonSet(a), p) || iter(a + 1)
  ...
  def map(s: Set, f: Int => Int): Set = {
    def iter(a: Int): Set = {
      if (a <= bound && contains(s, a)) union((x: Int) => x == f(a), iter(a + 1))
      else if (a > bound) (x: Int) => false
      else iter(a + 1)
    }
    iter(-bound)
  }
