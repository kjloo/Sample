/*  main.cpp: driver for os-wars problem
...
struct ThreadParam {
  ...
  enum FavoriteOS favoriteOS;	// favoriteOS
...
main(int argc, char *argv[])
{
  ...
  for (int i = 0; i < numThreads; i++) {
    ...
    paramPtr->id = i;
    switch (i % 3) {
      case 0: paramPtr->favoriteOS = OS_X; break;
      case 1: paramPtr->favoriteOS = LINUX; break;
      case 2: paramPtr->favoriteOS = WINDOWS; break;
    }
...
void *threadStart(void *param)
{
  ...
  int id = paramPtr->id;
  enum FavoriteOS favoriteOS = paramPtr->favoriteOS;
  ...
  switch (favoriteOS) {
    case OS_X:
      mdat_thread_start(id, "OS_X");
      break;
    case LINUX:
      mdat_thread_start(id, "Linux");
      break;
    case WINDOWS:
      mdat_thread_start(id, "Windows");
      break;
    default:
      cerr << "Error - incorrect favorite OS" << endl;
      exit(-1);
  }

  // Execute the sections
  for (int i = 0; i < numRounds; i++) {
    sectionEntrySection(favoriteOS);
    sectionCriticalSection();
    sectionExitSection(favoriteOS);
    sectionRemainderSection();
  }
