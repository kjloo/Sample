  ...
  //If none of the above try to fork a child process
	...
	if(pid < 0) { //Error Forking
	  ...
	  if(cmd.input_redirect) { //Redirect Input using Specified file
		//Open Input File
		fd = open(cmd.input_redirect, O_RDONLY, S_IRUSR | S_IWUSR);
		if(fd == -1) {
	  ...
	  }else if (cmd.output_redirect) { //Output Redirect using Specified File
		//Open or Create Output File 
		fd = open(cmd.output_redirect, O_WRONLY | O_CREAT | O_TRUNC,
				  S_IWUSR | S_IRUSR);
		if(fd == -1) {
		  cerr << "Error opening file: "  << strerror(errno) << endl;
		}else {
